#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Rewrite scalper_positions.py without Chinese"""
...
def adjust(pos, state):
    ...
    # Signal weak
    if pos.get("score_drop", 0) >= SIGNAL_WEAKEN_THRESHOLD and pnl < 0.03:
        qty = int(bal * 0.5)
        if qty > 0:
            ok, _ = market_sell(ca, qty, cid, 15)
            if ok:
                acts.append("REDUCED 50% (signal weak)")
...
def main():
    ...
    for p in positions:
        ...
        w = "⚠️" if p.get("score_drop", 0) >= SIGNAL_WEAKEN_THRESHOLD else ""
