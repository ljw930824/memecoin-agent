...
def get_wallet_balances():
    """Returns {by_ca, by_sym} for position lookups."""
    try:
...
def execute_swap_with_retry(qty, from_token, to_token, chain_id, slippage=3, max_retries=MAX_RETRY_ATTEMPTS):
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if success:
            ...
            log_retry(from_token[:8], "SWAP", str(result)[:50], attempt)
...
def market_sell_token(token_addr, qty, chain_id, slippage=10):
    """v3.1: Market sell token (for stop-loss execution)."""
...
# ─── POSITION MANAGER (v3.1 Enhanced) ────────────────────────────────────────
...
def adjust_position_dynamic(pos, balances, state):
    """Apply dynamic SL/TP adjustments + v3.1 time-based rules."""
    pnl_pct  = pos["pnl_pct"]
    ticker   = pos["ticker"]
    ca       = pos["ca"]
    ...
    if holding_hours >= MAX_HOLDING_HOURS:
        if abs(pnl_pct) < NO_MOVEMENT_THRESHOLD:
            # No significant movement after 24h -> reduce 50%
            sell_qty = int(token_bal * 0.50)
            if sell_qty > 0:
                actions.append(f"TIMEOUT_24H: reducing 50% (holding {holding_hours:.1f}h, P&L {pnl_pct*100:.1f}%)")
    ...
    if pnl_pct <= STOP_LOSS_PCT and now_hour >= SAME_DAY_SL_CUTOFF_HOUR:
        actions.append(f"SAME_DAY_CLOSE: stop-loss triggered, forcing close before day end")
        chain_usdt = BSC_USDT if chain_id == "56" else SOL_USDT
        ok, data = market_sell_token(token_bal, ca, chain_id, slippage=20)
        if ok:
            actions.append(f"LIQUIDATED: sold all {token_bal} {ticker} at market")
            if ca in state.get("positions", {}):
    ...
    # 4. Hard Stop-Loss at -8% (v3.1: market sell instead of limit order)
    if pnl_pct <= STOP_LOSS_PCT:
        actions.append(f"HARD_SL: stop-loss triggered ({pnl_pct*100:.1f}%)")
        # v3.1: Use market sell for guaranteed execution
        chain_usdt = BSC_USDT if chain_id == "56" else SOL_USDT
        ok, data = market_sell_token(token_bal, ca, chain_id, slippage=15)
        if ok:
    ...
    if pnl_pct >= TAKE_PROFIT_PCT:
        ...
        chain_usdt = BSC_USDT if chain_id == "56" else SOL_USDT
        ok, data = execute_swap_with_retry(token_bal, ca, chain_usdt, chain_id, slippage=5)
        if ok:
...
def main():
    now = datetime.now(timezone(timedelta(hours=8)))
    print(f"\n{'='*60}")
    print(f"SCALPER v3.1 | {now.strftime('%Y-%m-%d %H:%M:%S')} CST")
    print(f"{'='*60}")
    ...
    print(f"\n[POSITIONS] Dynamic SL/TP + Time-based rules check")
