...
TREND_FILE = os.path.join(DATA, 'sm_price_trends_dryrun.json')
    ...
    for ca, pos in list(positions.items()):
        ...
        if elapsed > LIMIT_ORDER_TIMEOUT:
            ...
            if token_bal > 0:
                ...
                ok, _ = bsc_market_sell(ca, token_bal)
...
def execute_sell(chain, token_ca, token_balance):
    ...
    if chain == 'bsc':
        ...
        return bsc_market_sell(token_ca, token_balance)
    ...
    out, _ = oc_run([
        ...
        '--readable-amount', str(token_balance),  # FIX:  int()0
...
def check_positions(positions, state=None):
    ...
    for ca, ratio, label in to_sell_partial:
        ...
        if sell_amount > 0:
            ...
            ok, _ = execute_sell(chain, ca, sell_amount)
